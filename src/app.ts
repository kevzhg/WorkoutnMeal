...
function refreshRecentTrainings(): void {
...
function renderTrainingDetail(training: Training): string {
...
function renderTrainingEditForm(training: Training): string {
    const trainingId = training.id || 'training';
    return `
...
function bindRecentTrainingEvents(): void {
...
function initWorkoutForm(): void {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        const training = {
            ...
            type: (form.querySelector('#workout-type') as HTMLSelectElement).value as TrainingType,
