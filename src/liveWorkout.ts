...
const DEFAULT_PROGRAMS: WorkoutProgramInput[] = [
    {
...
let builderCategory: ProgramType = 'push';
const CATEGORY_LABELS: Record<ProgramType, string> = { push: 'Push', pull: 'Pull', legs: 'Legs' };
const expandedCategories: Set<ProgramType> = new Set();
let libraryFilterCategory: ProgramType | 'all' = 'all';
...
function getRestSeconds(value?: number): number {
    return typeof value === 'number' && Number.isFinite(value) && value >= 0 ? value : DEFAULT_SET_REST_SECONDS;
}

function unlockRestAudioContext(): void {
    if (restAudioPrimed) return;
    const Ctx = (window as any).AudioContext || (window as any).webkitAudioContext;
    if (!Ctx) return;

    try {
        let ctx: AudioContext | null = restBeepCtx;
        if (!ctx) {
            ctx = new Ctx();
            restBeepCtx = ctx;
        }
...
async function ensureDefaultPrograms(): Promise<void> {
...
function renderProgramCards(): void {
    ...
    const categoryIcons: Record<ProgramType, string> = { push: '🔺', pull: '🔻', legs: '🦵' };

    const grouped: Record<ProgramType, typeof programs> = { push: [], pull: [], legs: [] };
    programs.forEach(p => grouped[p.name].push(p));

    container.innerHTML = (Object.keys(grouped) as ProgramType[]).map(cat => {
...
function bindProgramCardActions(container: Element): void {
    const startButtons = container.querySelectorAll('.start-program-btn');
    startButtons.forEach(btn => {
        btn.addEventListener('click', (e) => {
            const programId = (e.currentTarget as HTMLElement).getAttribute('data-program-id');
            if (programId) startWorkout(programId);
        });
    });
    ...
    detailButtons.forEach(btn => {
        btn.addEventListener('click', (e) => {
            const programId = (e.currentTarget as HTMLElement).getAttribute('data-program-id');
            if (programId) toggleProgramDetails(programId);
        });
    });
...
function bindCategoryToggles(container: Element): void {
    const toggles = container.querySelectorAll('.program-category-toggle');
    toggles.forEach(btn => {
        btn.addEventListener('click', (e) => {
            const category = (e.currentTarget as HTMLElement).getAttribute('data-category') as ProgramType;
            const body = container.querySelector(`.program-category-body[data-category-body="${category}"]`) as HTMLElement | null;
            const caret = (e.currentTarget as HTMLElement).querySelector('.program-category-caret') as HTMLElement | null;
            if (!body) return;
            ...
            if (caret) caret.textContent = nextExpanded ? '▲' : '▼';
...
function setActiveTab(tab: 'start' | 'build'): void {
    activeTab = tab;
    const startPanel = document.getElementById('tab-start');
    const buildPanel = document.getElementById('tab-build');
    const tabButtons = document.querySelectorAll('.tab-btn');

    if (startPanel) startPanel.classList.toggle('active', tab === 'start');
    if (buildPanel) buildPanel.classList.toggle('active', tab === 'build');
...
function setupExerciseLibrary(): void {
    ...
    chips.forEach(chip => {
        chip.addEventListener('click', (e) => {
            ...
            const category = (e.currentTarget as HTMLElement).getAttribute('data-library-category') as ProgramType | 'all';
    ...
    typeChips.forEach(chip => {
        chip.addEventListener('click', (e) => {
            ...
            (e.currentTarget as HTMLElement).classList.add('active');
            const type = (e.currentTarget as HTMLElement).getAttribute('data-library-type') as ExerciseType | 'all';
...
function computeProgramFocus(program: WorkoutProgram): { label: string; detail: string } {
    const priority: ExerciseType[] = ['power', 'hypertrophy', 'compound', 'flexibility', 'cardio'];
    ...
    program.exercises.forEach(ex => {
        const type = (ex.exerciseType ?? 'compound') as ExerciseType;
...
function toggleProgramDetails(programId: string): void {
    const details = document.querySelector(`.program-details[data-program-id="${programId}"]`) as HTMLElement | null;
    if (!details) return;
    const isHidden = details.style.display === 'none' || details.style.display === '';
    details.style.display = isHidden ? 'block' : 'none';

    const btn = document.querySelector(`.details-program-btn[data-program-id="${programId}"]`) as HTMLButtonElement | null;
    if (btn) {
        btn.textContent = isHidden ? 'Hide Details' : 'Details';
    }
}

function showLiveToast(message: string): void {
...
async function cloneProgram(programId: string): Promise<void> {
...
function setupProgramBuilder(): void {
    const addBtn = document.getElementById('builder-add-exercise') as HTMLButtonElement | null;
    const saveBtn = document.getElementById('builder-save') as HTMLButtonElement | null;
    const resetBtn = document.getElementById('builder-reset') as HTMLButtonElement | null;
    const cancelEditBtn = document.getElementById('builder-cancel-edit') as HTMLButtonElement | null;
    const segmented = document.querySelectorAll('#builder-segmented .segment-btn');
    const searchInput = document.getElementById('builder-exercise-search') as HTMLInputElement | null;

    segmented.forEach(btn => {
        btn.addEventListener('click', (e) => {
            const value = (e.currentTarget as HTMLElement).getAttribute('data-category') as ProgramType;
    ...
    searchInput?.addEventListener('input', (e) => {
        builderFilterExercises((e.target as HTMLInputElement).value || '');
    });
...
function setupLiveTabs(): void {
    const tabButtons = document.querySelectorAll('.tab-btn');
    tabButtons.forEach(btn => {
        btn.addEventListener('click', (e) => {
            const tab = (e.currentTarget as HTMLElement).getAttribute('data-tab') as 'start' | 'build';
            setActiveTab(tab);
        });
    });
...
function renderExerciseOptions(): void {
    const select = document.getElementById('builder-exercise') as HTMLSelectElement | null;
    if (!select) return;
...
function builderFilterExercises(term: string): void {
    const select = document.getElementById('builder-exercise') as HTMLSelectElement | null;
    if (!select) return;
...
function setBuilderEditingState(program: WorkoutProgram | null): void {
    const label = document.getElementById('builder-editing-label') as HTMLElement | null;
    const cancelBtn = document.getElementById('builder-cancel-edit') as HTMLButtonElement | null;
    const saveBtn = document.getElementById('builder-save') as HTMLButtonElement | null;
    const segmented = document.querySelectorAll('#builder-segmented .segment-btn');
    ...
    if (program) {
        ...
        segmented.forEach(btn => {
            const cat = (btn as HTMLElement).getAttribute('data-category');
            if (cat === program.name) btn.classList.add('active'); else btn.classList.remove('active');
        });
    } else {
        if (label) {
        ...
        segmented.forEach((btn, idx) => {
            if (idx === 0) btn.classList.add('active'); else btn.classList.remove('active');
        });
    }
}

function addExerciseToBuilder(): void {
    const select = document.getElementById('builder-exercise') as HTMLSelectElement | null;
    ...
    if (!select || !setsInput || !repsInput) return;
...
function addExerciseFromLibrary(exerciseId: string): void {
    ...
    const setsInput = document.getElementById('builder-sets') as HTMLInputElement | null;
    const repsInput = document.getElementById('builder-reps') as HTMLInputElement | null;
    if (setsInput) setsInput.value = String(template.sets);
    if (repsInput) repsInput.value = String(template.reps);
    const select = document.getElementById('builder-exercise') as HTMLSelectElement | null;
    if (select) select.value = template.id;
    addExerciseToBuilder();
}

function enterEditMode(programId: string): void {
    const program = getWorkoutProgramById(programId);
    if (!program) {
    ...
    const nameInput = document.getElementById('builder-name') as HTMLInputElement | null;

    builderCategory = program.name;
    if (nameInput) nameInput.value = program.displayName;
...
function renderBuilderExercises(): void {
    ...
    list.querySelectorAll('.move-down-btn').forEach(btn => {
        btn.addEventListener('click', (e) => {
            e.preventDefault();
            const index = parseInt((e.currentTarget as HTMLElement).getAttribute('data-index') || '0', 10);
            if (index >= builderExercises.length - 1) return;
            const [item] = builderExercises.splice(index, 1);
            builderExercises.splice(index + 1, 0, item);
            renderBuilderExercises();
        });
...
function updateBuilderSummary(): void {
    ...
    if (countEl) countEl.textContent = `${totalExercises} ${totalExercises === 1 ? 'exercise' : 'exercises'}`;
    if (setsEl) setsEl.textContent = `${totalSets} ${totalSets === 1 ? 'set' : 'sets'} total`;
}

function resetBuilderForm(): void {
    const nameInput = document.getElementById('builder-name') as HTMLInputElement | null;
    const setsInput = document.getElementById('builder-sets') as HTMLInputElement | null;
    const repsInput = document.getElementById('builder-reps') as HTMLInputElement | null;
    ...
    builderExercises = [];
    builderCategory = 'push';
    if (nameInput) nameInput.value = `${CATEGORY_LABELS['push']} Session`;
    if (setsInput) setsInput.value = '4';
    if (repsInput) repsInput.value = '8';
...
async function saveBuilderProgram(): Promise<void> {
    const nameInput = document.getElementById('builder-name') as HTMLInputElement | null;
...
function startWorkout(programId: string): void {
    const program = getWorkoutProgramById(programId);
...
function completeSet(exerciseIndex: number, setIndex: number): void {
    ...
    if (nextSetIndex !== -1) {
        ...
        nextExerciseIndex = activeWorkout.exercises.findIndex((ex, idx) => 
...
function startResting(seconds: number, label: string = REST_LABEL_DEFAULT, activeWorkoutOverride?: ActiveWorkout): void {
